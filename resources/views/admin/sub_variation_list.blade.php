                ...
                <div class="row">
                  <div class="col-1">
                    <div class="d-flex justify-content-center align-items-center" style="background-color: tomato;border-radius: 50%; width: 50px; height: 50px; text-align: center;">
                      <a href="/admin/sub_variation/create"><i class="fas fa-plus" style="font-size: 30px; color:#fff;"></i></a>
                    </div>
                  </div>
                  <div class="col-10">
                ...
                <table id="example1" class="table table-bordered">
                  ...
                  <tbody>
                    ...
                    <td>{{ $loop->index+1 }}</td>
                     ...
                     <td>{{ $sub_variations->variation->variation_name  }}</td>
                      <td>{{ $sub_variations->sub_variation_name   }}</td>
                    
                        <td>{{ $sub_variations->status }}</td>
                      
                   
                      <td>
                          <a href="{{ route('sub_variation.edit', $sub_variations->id) }}" title='Update'><button class='btn btn-xs btn-warning mr-2 p-1'><i class='fas fa-edit' ></i> </button></a>
                              
          <form method="POST" action="{{ route('sub_variation.destroy', $sub_variations->id) }}" class="d-inline">
            @csrf
            @method('DELETE')
