                ...
                <div class="row">
                  <div class="col-1">
                    <div class="d-flex justify-content-center align-items-center" style="background-color: tomato;border-radius: 50%; width: 50px; height: 50px; text-align: center;">
                      <a href="{{ route('subscriber.create') }}"><i class="fas fa-plus" style="font-size: 30px; color:#fff;"></i></a>
                    </div>
                  </div>
                  <div class="col-11">
                ...
                <table id="example2" class="table table-bordered">
                  ...
                  <tbody>
                     ...
                     @foreach( $subscribers as $subscribers)
                      <tr>
                    <td>{{ $loop->index+1 }}</td>
                      <td>{{ $subscribers->subscribers_email }}</td>
                    
                        <td>{{ $subscribers->status }}</td>
                      
                   
                      <td>
                          <a href="{{ route('subscriber.edit',$subscribers->id) }}" title='Update'><button class='btn btn-xs btn-warning mr-2 p-1'><i class='fas fa-edit' ></i> </button></a>
                              
          <form method="POST" action="{{ route('subscriber.destroy',$subscribers->id) }}" class="d-inline">
            @csrf
            @method('DELETE')
